...
public class LSJResourceConverter : JsonConverter
{
    ...
    {
        if (objectType == typeof(Node) && existingValue is Node node)
        {
            return ReadNode(reader, node);
        }

        if (objectType == typeof(Resource) && existingValue is Resource resource)
        {
            return ReadResource(reader, resource);
        }

        throw new InvalidOperationException("Cannot deserialize unknown type");
    }
    ...
    private Resource ReadResource(JsonReader reader, Resource? resource)
    {
        ...
        while (reader.Read())
        {
            ...
            switch (reader)
            {
                case { TokenType: JsonToken.PropertyName }:
                    key = reader.Value.ToString();
                    break;

                case { TokenType: JsonToken.StartObject }:
                {
    ...
    private void WriteResource(JsonWriter writer, Resource resource)
    {
    ...
    private void WriteNode(JsonWriter writer, Node? node)
    {
        writer.WriteStartObject();
